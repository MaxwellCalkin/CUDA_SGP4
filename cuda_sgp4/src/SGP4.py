...
def dpper(e3,ee2,peo, pgho, pho, pinco, plo, se2, se3, sgh2,
    ...
    if init == 'n':
      ...
      if rec.inclp >= 0.2:
        ...
        alfdp = alfdp + dalf
        betdp = betdp + dbet
        rec.nodep = math.fmod(rec.nodep, twopi)
        #//  sgp4fix for afspc written intrinsic functions
        #// nodep used without a trigonometric function ahead
        if (rec.nodep < 0.0) and (opsmode == 97.0):
        ...
        if (rec.nodep < 0.0) and (opsmode == 97.0):
          rec.nodep = rec.nodep + twopi
        if math.fabs(xnoh - rec.nodep) > pi:
          if (rec.nodep < xnoh):
            rec.nodep = rec.nodep + twopi
          else:
            rec.nodep = rec.nodep - twopi
...
def dscom (epoch, ep, argpp, tc, inclp, nodep, np, rec):
    ...
    xnodce = math.fmod(4.5236020 - 9.2422029e-4 * rec.day, twopi)
    ...
    rec.zmol = math.fmod(4.7199672 + 0.22997150  * rec.day - rec.gam, twopi)
    rec.zmos = math.fmod(6.2565837 + 0.017201977 * rec.day, twopi)
...
def dsinit(tc, xpidot, rec):
    ...
    if (rec.irez != 0):
      ...
      if (rec.irez == 2):
        ...
        rec.xlamo = math.fmod(rec.mo + rec.nodeo + rec.nodeo - theta - theta, twopi)
      ...
      if (rec.irez == 1):
        ...
        rec.xlamo = math.fmod(rec.mo + rec.nodeo + rec.argpo - theta, twopi)
...
def dspace(tc, rec):
    ...
    if (rec.irez != 0):
      ...
      if ((rec.atime == 0.0) or (rec.t * rec.atime <= 0.0) or (math.fabs(rec.t) < math.fabs(rec.atime))):
        rec.atime = 0.0
        rec.xni = rec.no_unkozai
        rec.xli = rec.xlamo
      ...
      while (iretn == 381):
        ...
        if (math.fabs(rec.t - rec.atime) >= stepp):
          iretn = 381
        else: #// exit here
          ft = rec.t - rec.atime
          iretn = 0
...
def initl(epoch, rec):
    ...
    c1p2p = c1 + twopi
    gsto1 = math.fmod(thgr70 + c1*ds70 + c1p2p*tfrac + ts70*ts70*fk5r, twopi)
    if (gsto1 < 0.0):
      gsto1 = gsto1 + twopi
    #//    }
    #//    else
...
def sgp4init(opsmode, satrec):
    ...
    if ((satrec.omeosq >= 0.0) or (satrec.no_unkozai >= 0.0)):
      ...
      if (math.fabs(satrec.cosio + 1.0) > 1.5e-12):
...
def sgp4(satrec, tsince, r, v):
    ...
    if ((satrec.em >= 1.0) or (satrec.em < -0.001)):  # /* or (am < 0.95)*/
        satrec.error = 1
    ...
    satrec.nodem = math.fmod(satrec.nodem, twopi)
    satrec.argpm = math.fmod(satrec.argpm, twopi)
    xlm = math.fmod(xlm, twopi)
    satrec.mm = math.fmod(xlm - satrec.argpm - satrec.nodem, twopi)
    ...
    if (satrec.method == 'd'):
        ...
        if (math.fabs(cosip + 1.0) > 1.5e-12):
    ...
    u = math.fmod(xl - satrec.nodep, twopi)
    eo1 = u
    tem5 = 9999.9
    ktr = 1
    sineo1 = 0
    ...
    while ((math.fabs(tem5) >= 1.0e-12) and (ktr <= 10)):
        sineo1 = math.sin(eo1)
        coseo1 = math.cos(eo1)
        tem5 = 1.0 - coseo1 * axnl - sineo1 * aynl
        tem5 = (u - aynl * coseo1 + axnl * sineo1 - eo1) / tem5
        if (math.fabs(tem5) >= 0.95):
            if (tem5 > 0):
                tem5 = 0.95
            else:
                tem5 = -0.95
...
@cuda.jit
def cudaSGP4 (satrec,tsince,r,v):
...
def getgravconst(whichconst, rec):
...
def gstime(jdut1):
    ...
    temp = math.fmod(temp * deg2rad / 240.0, twopi) #//360/86400 = 1/240, to deg, to rad
